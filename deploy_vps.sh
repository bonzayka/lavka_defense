...
echo "    Для автозапуска как systemd-сервисы (бот + микросервис инференса):"
echo "        sudo cp defense-bot.service /etc/systemd/system/"
echo "        sudo cp defense-inference.service /etc/systemd/system/"
echo "        sudo systemctl daemon-reload && sudo systemctl enable --now defense-inference defense-bot"
echo ""
