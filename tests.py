...
# Тесты на защиту от ложных срабатываний (рекламные ссылки, ref-коды, боты, суммы, ID):
_user_spam_msg = (
    "https://t.me/vakansii11_bot?start=ref7475771830\n"
    "Привет! Я нашел классное приложение, с помощью которого можно быстро заработать всего за несколько кликов. "
    "Вот ссылка, все проверено: 👉 https://t.me/vakansii11_bot?start=ref7475771830 . В первый день можно заработать от 2000 рублей!"
)
check("deanon-текст: реклама с ref-ссылкой и ботом НЕ деанон", not deanon.scan_text(_user_spam_msg, 2)[0])
check("deanon-текст: ссылка с 10-значным ID НЕ деанон", not deanon.scan_text("https://t.me/cryptobot?start=1234567890", 2)[0])
check("deanon-текст: заказ и дата/время НЕ деанон", not deanon.scan_text("Заказ №8492019381 отправлен 2026-09-24 15:48:37.", 2)[0])
check("deanon-текст: URL статьи с цифрами НЕ деанон", not deanon.scan_text("Смотри https://habr.com/ru/articles/1234567890/", 2)[0])
check("deanon-текст: криптокошелек НЕ деанон", not deanon.scan_text("USDT TRC20: TNPeeDeY7q9Q7fGqV4gR8X1w9d2Y6t8m3k", 2)[0])
check("deanon-текст: разговорные фразы со словами 'паспорт/телефон' НЕ деанон",
      not deanon.scan_text("потерял паспорт, напиши мне в лс свой ник", 2)[0])
# Тесты на детекцию реальных/сгенерированных ПДн (SXRandomDataTools):
_fake_card = (
    "ФИО: Фадеева Ева Прохоровна\n"
    "Паспорт: 6501 429135\n"
    "Телефон: +7 (803) 844-87-88\n"
    "Адрес: Россия, г. Москва, Озерная ул., 36, корп. 3, кв. 156\n"
    "ИНН: 689443369808\n"
    "СНИЛС: 112-233-445 95"
)
check("deanon-текст: генератор SXRandomDataTools карточка ПДн срабатывает", deanon.scan_text(_fake_card, 2)[0])
check("deanon-текст: генератор SXRandomDataTools паспорт (тяжёлый) срабатывает", deanon.scan_text("паспорт 6501 429135", 2)[0])
check("deanon-текст: генератор SXRandomDataTools телефон+адрес срабатывает",
      deanon.scan_text("номер +7 (803) 844-87-88, адрес Россия, г. Москва, Озерная ул., 36, корп. 3, кв. 156", 2)[0])
check("deanon-текст: одиночный номер +79954226469 ловится", deanon.scan_text("+79954226469", 2)[0])
check("deanon-текст: одиночный номер без плюса 79954226469 ловится", deanon.scan_text("79954226469", 2)[0])

_osint_dump_card = """👤 Основные данные
├ ФИО: Симагулин Фарий Равильевич
├ Дата рождения: 25.02.1975
└ Возраст: 51"""
check("deanon-текст: карточка пробива/OSINT-дамп ловится", deanon.scan_text(_osint_dump_card, 2)[0])
check("deanon-текст: одиночное 3-составное ФИО ловится", deanon.scan_text("Симагулин Фарий Равильевич", 2)[0])
check("deanon-текст: обратный порядок ИОФ ловится", deanon.scan_text("Фарий Равильевич Симагулин", 2)[0])
check("deanon-текст: ФИО с меткой Ф.И.О. ловится", deanon.scan_text("Ф.И.О.: Симагулин Фарий Равильевич", 2)[0])
check("deanon-текст: ФИО с меткой фио в нижнем регистре ловится", deanon.scan_text("фио: симагулин фарий равильевич", 2)[0])
check("deanon-текст: ФИО раздельное (Фамилия / Имя / Отчество) ловится",
      deanon.scan_text("Фамилия: Симагулин\nИмя: Фарий\nОтчество: Равильевич", 2)[0])
check("deanon-текст: ФИО + дата рождения ловится",
      deanon.scan_text("Симагулин Фарий Равильевич 25.02.1975", 2)[0])
check("deanon-текст: классики литературы НЕ деанон", not deanon.scan_text("Александр Сергеевич Пушкин", 2)[0])
check("stopword: цифровой ID 1821853339 ловится через стоп-слова",
      textguard.find_stopword("1821853339", ["1821853339"]) == "1821853339")
...
# ---- AdGuard: детект 18+ ботов, накрутки отзывов и спам-заработка ----
SPAM_SAMPLES = [
    ("Оставить 10 отзывов на сайте с разных аккаунтов которые выдам\nна карту 2500", "отзывы за деньги"),
    ("𓆩♡𓆪 реальная, пишите 𓆩♡𓆪\n💋 недорого — докажу в лс☑️\n🔥 много горячих услуг 🔥\n🦋 пиши, не стесняйся 🫦", "18+ услуги"),
    ("Кто хочет оценить меня и моих подруг?🥰", "18+ оценка"),
    ("Привет я Ева💋Шалю по онлайну❤️виdео и фото💕жду тебя🥰", "18+ шалю/вирт"),
    ("ПОМОГУ ДОТЯНУТЬ ДО ЗАРПЛАТЫ!", "дотянуть до зп"),
    ("мальчики, интересует онлайн рабыня? 18 лет, отличная форма))🎭🎭🎭", "18+ рабыня"),
    ("Есть тут любители оценивать фото? Если да, пишите жду вашей оценки🥰😘", "18+ оценка фото"),
    ("💗🌸 Проверенная девочка 🌸💗\n💕 Нежная, сочная и очень горячая — исполню твои желания\n💎 Живое общение • видеозвонки • приват\n💗 Личный архив, кастом и много эксклюзива\n💦 Сделаю именно так, как нравится тебе — без лишних запретов 😈\n💌 Полный прайс и все подробности — в ЛС", "18+ анкета"),
    ("💖 Реальная девочка, пишите мне 💖\n💕Фото • 👾Видео • 🔥 Видеозвонки\n💋 Эксклюзив • 💌Вирт • 🔒🐈Приват\n🫦Большой прайс 🫦\n\n♡₊˚ 💌 Канал с предпоказом в ЛС 💌 ˚₊♡", "18+ прайс/вирт"),
    ("Помогу заработать за пол дня 15 000 . Кому интересно пишите в личные сообщения", "заработок 15000"),
    ("Есть дельце на 2 часа, дам 4000р сразу", "дам 4000р сразу"),
    ("Учимся зарабатывать дома\nЕсли есть моб интернет и нормальный телефон пиши мне все покажу расскажу\nот 6 000 за вечер", "заработок от 6000"),
    ("пomогy kончuть в лc", "18+ гомоглифы"),
    ("У меня в канале мало денег не бывает. Подписывайся на канал в профиле 💰", "канал в профиле"),
    ("Работа на пляже\nукладка берега песком\nв день от 15 000", "работа от 15000"),
    ("Не упускай шанса потому что потом будешь жалеть. 6к рублей за 1.5 часа.", "6к за 1.5 часа"),
    ("Обучу новому искусству любви Бесплатно))) Жду 💋💋💋", "18+ искусство любви"),
    ("Осавить отзыв у меня на канале такой который я скажу\n2000 на карту", "отзыв 2000 на карту"),
    ("Есть варианты для тех кто ищет дополнительный доход", "дополнительный доход"),
    ("Ищу ценителей Вирта, большой ассортимент 🤤🥰😘", "18+ вирт ассортимент"),
    ("Нету $? Не проблема, есть тема где за час 21 000р. В неделю больше, напиши узнаешь))", "за час 21000р"),
    ("Не работай за копейку, заработай себе КОТЛЕТУ‼️ ❤️ Переходи в канал", "заработай котлету"),
    ("🟢 НУЖНЫ 2 ЧЕЛОВЕКА НА ПОДРАБОТКУ 🟢\n\nПеренести обувь со склада 📦\nРаботы примерно на пару часов\n\n💰 Оплата сразу после работы\n10 000р/день\n\nКому интересно — пишите 😊\nЗаявка в БИО по ссылке", "склад/подработка в био"),
    ("Есть груз который нужно доставить в другой город. 40.000р+бензин. Наличие авто обязательно", "груз / авто"),
...
for text, label in SPAM_SAMPLES:
    hit, reason = adguard.check_spam(text)
    check(f"adguard spam: {label}", hit)

HAM_SAMPLES = [
    ("Ой, ну и реклама у нас тут пошла. Котлеты конечно заманчиво, но я как-то больше по общению, чем по таким предложениям :)", "реплика Розы"),
    ("Ого, тут у нас прямо настоящий рынок услуг открылся. Кажется, кто-то очень хочет привлечь внимание.", "реплика Розы 2"),
    ("Спб, мне 18, учусь на 2 курсе в колледже, ищу здесь отношения, ну можно и пообщаться(в приоритете отношения), живу на елизаровской, всем хорошего дня и настроения)))", "анкета Игната"),
    ("Виктория: Ищу с кем пообщаться😘", "общение Виктория"),
    ("Ого, сколько тут рекламных объявлений сегодня. Даже читать глаза режет от такого количества сердечек.", "реплика Розы 3"),
    ("> 💗🌸 Проверенная девочка 🌸💗\nУжас какой, админы удалите", "цитирование спама в ответе"),
    ("Привет, как дела? Погнали сегодня в кино вечером?", "обычное сообщение"),
    ("Кто умеет писать ботов на Python? Нужна помощь с aiogram", "вопрос по коду"),
]

for text, label in HAM_SAMPLES:
    hit, reason = adguard.check_spam(text)
    check(f"adguard ham: {label}", not hit)
